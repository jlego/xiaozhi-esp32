...
void LcdDisplay::SetupUI() {
    ESP_LOGI(TAG, "SetupUI: text_font=%p, icon_font=%p, emoji_font=%p", fonts_.text_font, fonts_.icon_font, fonts_.emoji_font);
    DisplayLockGuard lock(this);

    auto screen = lv_screen_active();
    ESP_LOGI(TAG, "SetupUI: screen=%p", screen);
    if (fonts_.text_font && screen) {
        lv_obj_set_style_text_font(screen, fonts_.text_font, 0);
    } else {
        ESP_LOGE(TAG, "SetupUI: text_font or screen is NULL!");
    }
    if (fonts_.text_font && screen) {
    ...
    ESP_LOGI(TAG, "SetupUI: container_=%p", container_);
    if (container_) {
        ...
        ESP_LOGE(TAG, "SetupUI: container_ is NULL!");
    }
    ...
    lv_timer_create([](lv_timer_t * t) {
        lv_obj_invalidate(lv_scr_act()); // 标记全屏无效
